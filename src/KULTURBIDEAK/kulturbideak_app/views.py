...
'''
    ...
    **************************************************
'''
def ajax_egunekoak(request):
    #Egunekoak
    (eguneko_itemak,eguneko_ibilbideak)=get_egunekoak()  
    return render_to_response('ajax/ajax_egunekoak.html',{'eguneko_itemak':eguneko_itemak,
                                                          'eguneko_ibilbideak':eguneko_ibilbideak},context_instance=RequestContext(request))
...
@login_required
def ajax_egunekoa_aldatu_item(request):
    item_id = request.GET.get("id")
    itype = request.GET.get("itype")
    aldaketa = request.GET.get("aldaketa")
        #GURI ALDAKETAREN BERRI EMAN?    
    mezua="Hornitzailearen izena:"+str(request.user.username)+".\n"+"Item hau (id): "+str(item_id)+" egunekoetatik "+aldaketa+" du.\n"+"Beharra badago bidali mezua hornitzaileari: "+str(request.user.email)
    if aldaketa == "gehitu":
        item.objects.filter(id=item_id).update(egunekoa = 1)
    elif aldaketa == "kendu":
        item.objects.filter(id=item_id).update(egunekoa = 0)
    else:
        mezua="Hornitzailearen izena:"+str(request.user.username)+".\n"+"Item hau (id): "+str(item_id)+" egunekoetatik "+aldaketa+" nahi izandu baina errorea gertatu da.\n"+"Beharra badago bidali mezua hornitzaileari: "+str(request.user.email)
        
    send_mail('OndareBideak - Eguneko itemetan aldaketak', mezua, 'ondarebideak@elhuyar.eus',['ondarebideak@elhuyar.eus'], fail_silently=False)
    
        
    return render_to_response("ajax/ajax_egunekoa_aldatu_item.html",{"aldaketa": aldaketa,
                                                                     "id":item_id,
                                                                     "itype": itype},context_instance=RequestContext(request))   


#EGUNEKO IBILBIDEA KUDEATZEKO BI FUNTZIO    
@login_required
def ajax_egunekoa_aldatu_path(request):
    path_id = request.GET.get("id")
    aldaketa = request.GET.get("aldaketa")
        #GURI ALDAKETAREN BERRI EMAN?    
    mezua="Hornitzailearen izena:"+str(request.user.username)+".\n"+"Ibilbide hau (id): "+str(path_id)+" egunekoetatik "+aldaketa+" du.\n"+"Beharra badago bidali mezua hornitzaileari: "+str(request.user.email)
    if aldaketa == "gehitu":
        path.objects.filter(id=path_id).update(egunekoa = 1)
    elif aldaketa == "kendu":
        path.objects.filter(id=path_id).update(egunekoa = 0)
    else:
        mezua="Hornitzailearen izena:"+str(request.user.username)+".\n"+"Ibilbide hau (id): "+str(path_id)+" egunekoetatik "+aldaketa+" nahi izandu baina errorea gertatu da.\n"+"Beharra badago bidali mezua hornitzaileari: "+str(request.user.email)
        
    send_mail('OndareBideak - Eguneko ibilbidetan aldaketak', mezua, 'ondarebideak@elhuyar.eus',['ondarebideak@elhuyar.eus'], fail_silently=False)
    
        
    return render_to_response("ajax/ajax_egunekoa_aldatu_path.html",{"aldaketa": aldaketa,
                                                                     "id":path_id},context_instance=RequestContext(request))   
...
def autoplay_hasieratik(request):
    ...
    if 'path_id' in request.GET:
        ...
        non="fitxaE"
        return render_to_response('navigator.html',{'non':non,
                                                    "mlt":mlt,
                                                    "comment_form": comment_form,
                                                    "comment_parent_form": comment_parent_form,
                                                    "comments": comments,
...
def nabigatu(request):
    ...
    if(autoplay == 1):
        non="fitxaE"
        return render_to_response('navigator.html',{"mlt":mlt,
                                                    "non":non,
                                                    "comment_form": comment_form,
                                                    "comment_parent_form": comment_parent_form,
                                                    "comments": comments,
